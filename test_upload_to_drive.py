...
def reset_global_cache():
    """Resets the global service cache before and after every test."""
    upload_to_drive._service_cache = None
    yield
    upload_to_drive._service_cache = None
...
def test_upload_file_success(
    mock_media_file_upload, mock_build, mock_auth_default, mock_file
):
    ...
    mock_media_file_upload.assert_called_once_with(
        mock_file, mimetype="text/plain", resumable=True
    )
...
@patch("upload_to_drive.build")
@patch("upload_to_drive.MediaFileUpload")
def test_upload_file_default_name_and_mime(
    mock_media_file_upload, mock_build, mock_auth_default, tmp_path
):
...
@patch("upload_to_drive.build")
@patch("upload_to_drive.MediaFileUpload")
def test_upload_file_caches_service(
    mock_media_file_upload, mock_build, mock_auth_default, mock_file
):
    """Tests that subsequent upload calls reuse the cached service object."""
    mock_creds = MagicMock()
    mock_auth_default.return_value = (mock_creds, "test-project")
    ...
    mock_create = MagicMock()
    mock_files.create.return_value = mock_create
    mock_create.execute.return_value = {"id": "some-id"}

    # Run upload twice
    result1 = upload_file(mock_file, "file1.txt")
    result2 = upload_file(mock_file, "file2.txt")

    assert result1 == "some-id"
    assert result2 == "some-id"

    # build() and google.auth.default() should be called exactly once
    mock_auth_default.assert_called_once()
    mock_build.assert_called_once_with("drive", "v3", credentials=mock_creds)

    # But files and create should be called twice since we upload twice
